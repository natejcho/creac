    ...
    fs.createDirectorys(componentPath)
      .then(() => {
        Promise.all([
          ...
          fs.writeFileAsync(path.join(componentPath, `${componentName}.stories.mdx`), createStories(componentName)),
        ]).then(() => resolve([`${componentName}.js`, `${componentName}.spec.js`, `${componentName}.stories.mdx`]));
      })
      .catch(e => {
        logger.error(e);
        process.exit();
